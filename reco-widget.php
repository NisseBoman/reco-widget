...
Version: 0.3
Author: Angry Creative AB, Plux Stahre, WilsonCreative
	...
	if(is_array($data)) {
		foreach(array_keys($data) as $i) {
          ...
          //$buf .= '        </span> skrev om <span class="reco_company_name"><a href="'. $itemList->restfulIdentifier .'">'. $itemList->name .'</a></span>: <br/>';
						$buf .= '        </span> skrev om <span class="reco_company_name"><a href="https://www.reco.se/'. $data[$i]->venueMeta->venueIdentifier .'/'. $data[$i]->employeeMeta->employeeIdentifier .'">'. $data[$i]->employeeMeta->employeeName .'</a></span>: <br/>';
            $buf .=  $data[$i]->text;
            $buf .= '    </div>';
